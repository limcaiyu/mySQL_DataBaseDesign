CREATE DATABASE printsIII;
USE printsIII;

Create table customers(customer_id integer,customer_name char(25),customer_add char(75),customer_city char(20),customer_state char(4),customer_zip integer,customer_phone char(20));
CREATE TABLE items(item_id integer, Title char(50), artist char(50),unit_price float, on_hand integer);
...
-- Assignment 3 
/* Create a view named Under_100. It consists of the item_id, title, artist, unit_price and order_qty 
for every print with a unit_price under 100 dollars.   */
CREATE VIEW Under_100 AS
SELECT items.item_id, title, artist, unit_price, order_qty
FROM items, orders, orderline
WHERE items.item_id = orderline.item_id AND 
orders.order_id = orderline.order_id
AND unit_price < 100.00;

/* Create a view named Allen. It consists of the customer_id, customer_name, customer_phone, title, 
and artist of each print ordered.    */
...
WHERE customers.customer_id = 1500 AND orders.customer_id = 1500
AND orders.order_id = orderline.order_id
AND orderline.item_id = items.item_id
GROUP BY items.artist;


/* Create a view named orders. It consists of the item_id, title, artist, unit_price and order_qty for 
every print ordered in the range of 2014-01-01 and 2014-02-28.  */
CREATE VIEW orders_view AS 
SELECT items.item_id, Title, artist, unit_price, order_qty 
FROM items,orderline,orders
WHERE items.item_id = orderline.item_id AND orders.order_id = orderline.order_id 
AND orders.order_date between '2014-01-01' and '2014-02-28'
GROUP BY items.item_id;


/* Create a view named zip_27. It consists of the customer_name, customer_phone, title, artist 
and date_shipped of each print ordered by a customer whose zip code begins with 27.    */
CREATE VIEW zip_27 AS
SELECT customers.customer_id, customers.customer_name, customers.customer_phone, Title, artist, ship_date
