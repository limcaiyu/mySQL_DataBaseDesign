...
CREATE VIEW Allen2 AS
  SELECT customers.customer_id, customer_name, customer_phone, title, artist
  FROM (((customers INNER JOIN orders ON customers.customer_id = orders.customer_id) 
  JOIN orderline ON orderline.order_id = orders.order_id) JOIN items ON items.item_id = orderline.item_id); 
  
CREATE VIEW orders AS 
SELECT i.item_id, i.Title, i.artist, i.unit_price, ol.order_qty 
FROM items i join orderline ol on i.item_id = ol.item_id join orders o on ol.order_id = ol.order_id 
WHERE o.order_date between '2014-01-01' and '2014-02-28'; 
...
SELECT customer_name, customer_zip, customer_state 
FROM customers
...
SELECT Title, (unit_price * 2) AS retail_price 
FROM items;

SELECT customer_name, customer_phone, order_date
FROM customers, orders
WHERE customers.customer_id = orders.customer_id AND orders.order_date >='2014-01-01' AND orders.order_date <='2014-12-31';

SELECT artist, SUM(order_qty) AS number_of_prints_sold
...
SELECT customer_name, Title, artist, SUM(order_qty)
FROM customers, items, orders, orderline
WHERE customers.customer_id = orders.customer_id
AND orders.order_id = orderline.order_id 
AND orderline.item_id = items.item_id 
